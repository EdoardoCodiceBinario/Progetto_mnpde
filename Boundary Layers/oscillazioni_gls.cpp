/*
File con soluzione (si guardi file parametri) senza alcun tipo di stabilizzazione,
il suo file di parametri associato è "parametri_oscillazioni_2d.prm".
In questo file si risolve il problema della oscillazioni mostrato nel file oscillazioni.cpp
mediante la stabilizzazione con metodo Galerkin Least Square.
*/
...
struct DiffTrasParameters
{
  DiffTrasParameters()
  {
    ...
    try
      {
        prm.parse_input("parametri_oscillazioni_" + std::to_string(dim) + "d.prm");
      }
    catch (std::exception &exc)
      {
        prm.print_parameters("parametri_oscillazioni_" + std::to_string(dim) + "d.prm");
        prm.parse_input("parametri_oscillazioni_" + std::to_string(dim) + "d.prm");
      }
            ...
            for (const unsigned int j : fe_values.dof_indices())
              {
               ...
               fe_values.JxW(q_index);                                  // dx 
                ...
                cell_matrix(i,j)+=
                tau*
                (b_coefficient(x_q) * fe_values.shape_grad(i,q_index))*  
                (b_coefficient(x_q) * fe_values.shape_grad(j,q_index))*
                fe_values.JxW(q_index);     
...
void DiffTras<dim>::output_results(const unsigned int cycle) const
{
  ...
  auto fname =
    "solution-oscillazione-gls-" + std::to_string(dim) + "d_" + std::to_string(cycle) + ".vtu";
  ...
  std::ofstream pvd_output("solution-oscillazione-gls-" + std::to_string(dim) + "d.pvd");
