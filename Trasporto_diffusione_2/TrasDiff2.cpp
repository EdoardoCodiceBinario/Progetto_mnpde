...
u(x,y)=exp(-(x+y)/a).
Il problema necessita di stabilizzazione e per valori del parametro dell'ordine di e-2/e-1 
non arriva a convergenza iterativa.
...
Trasdiff<dim>::assemble_system()
{
  ...
  std::cout << "Diffusione coefficiente: " << par.diffusione_coefficiente << std::endl;
            ...
            for (const unsigned int j : fe_values.dof_indices())
              {
               ...
               cell_matrix(i, j) +=
               par.diffusione_coefficiente*(fe_values.shape_grad(j, q_index) * // grad phi_i(x_q)
...
Trasdiff<dim>::output_results(const unsigned int cycle) const
{
  ...
  auto fname =
    "solution-dt2-" + std::to_string(dim) + "d_" + std::to_string(cycle) + ".vtu";
  ...
  std::ofstream pvd_output("solution-dt2-" + std::to_string(dim) + "d.pvd");
