      ...
      for (const unsigned int i : fe_values.dof_indices())
          {
            ...
            bnorm=b_coefficient(x_q).norm();
            bnorm=par.trasporto_coefficiente*bnorm;
            peclet=(h*bnorm)/(2*par.diffusione_coefficiente);  
            tau=(h/(2*bnorm))*funzione_peclet(peclet);

            for (const unsigned int j : fe_values.dof_indices())
              {
                lap_j=trace(fe_values.shape_hessian(j,q_index));
              ...
              // termine reazione  
               ...
               tau*
               lap_j*
               (par.trasporto_coefficiente*b_coefficient(x_q) * fe_values.shape_grad(i,q_index))*  
               fe_values.JxW(q_index); 

                cell_matrix(i,j)+=
                tau*
                (par.trasporto_coefficiente*b_coefficient(x_q) * fe_values.shape_grad(i,q_index))*  
                (par.trasporto_coefficiente*b_coefficient(x_q) * fe_values.shape_grad(j,q_index))*
                fe_values.JxW(q_index); 

                cell_matrix(i,j)+=
                tau*
                (par.trasporto_coefficiente*b_coefficient(x_q) * fe_values.shape_grad(j,q_index))*
                c_coefficient(x_q)*fe_values.shape_value(i,q_index)*
                fe_values.JxW(q_index); 

                cell_matrix(i,j)+=
                tau*
                (par.trasporto_coefficiente*b_coefficient(x_q) * fe_values.shape_grad(i,q_index))*
                c_coefficient(x_q)*fe_values.shape_value(j,q_index)*
                fe_values.JxW(q_index); 
...
Trasdiff<dim>::output_results(const unsigned int cycle) const
{
  ...
  auto fname =
    "solution-comparison-gls-" + std::to_string(dim) + "d_" + std::to_string(cycle) + ".vtu";
  ...
  std::ofstream pvd_output("solution-comparison-gls-" + std::to_string(dim) + "d.pvd");
